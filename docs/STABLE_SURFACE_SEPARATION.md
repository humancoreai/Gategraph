...
For the stable state, public and release surfaces must name `v0.17.3_CANDIDATE` as the current release and `v0.17.3_CANDIDATE` as the base.

The future stable token `v0.17.3_STABLE` may appear only as a future/promotion target, not as the current release.
