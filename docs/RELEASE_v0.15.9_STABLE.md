...
v0.17.7_CANDIDATE closes the stale-token window found during practical controlled-apply testing. When a rule is hardened through controlled apply, active non-expired capability tokens whose issuing decision depended on that rule are revoked.
...
v0.17.7_CANDIDATE is based on v0.17.7_CANDIDATE and keeps the single-node local protected deployment boundary unchanged.
