...
Candidate surfaces must name `v0.16.5_CANDIDATE` as the current release and `v0.16.5_CANDIDATE` as the base.

`v0.16.5_CANDIDATE` is only a future promotion target and must not appear as the current public release while metadata status is `stable`.
