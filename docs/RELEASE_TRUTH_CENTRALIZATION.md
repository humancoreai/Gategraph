...
Candidate surfaces must name `v0.16.3_STABLE` as the current release and `v0.16.3_STABLE` as the base.

`v0.16.3_STABLE` is only a future promotion target and must not appear as the current public release while metadata status is `stable`.
