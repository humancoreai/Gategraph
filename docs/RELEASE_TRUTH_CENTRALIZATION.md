...
Candidate surfaces must name `v0.16.1_CANDIDATE` as the current release and `v0.16.1_STABLE` as the base.

`v0.16.1_CANDIDATE` is only a future promotion target and must not appear as the current public release while metadata status is `stable`.
