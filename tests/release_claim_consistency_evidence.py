...
EXPECTED_RELEASE = "v0.14.10_CANDIDATE"
EXPECTED_BASE = "v0.14.9_STABLE"
EXPECTED_STATUS = "candidate"
EXPECTED_PHASE = "Public surface cleanup and review readiness"
SURFACES = [
    "README.md",
    "VERSION.md",
    "RELEASE_NOTES.md",
    "RELEASE_STATUS.md",
    "CHANGELOG.md",
    "docs/RELEASE_v0.14.10_CANDIDATE.md",
]
...
def main() -> int:
    ...
    checks = []
    checks.append(check("metadata_claims_current_candidate", meta.get("release") == EXPECTED_RELEASE and meta.get("base") == EXPECTED_BASE and meta.get("status") == EXPECTED_STATUS, {"release": meta.get("release"), "base": meta.get("base"), "status": meta.get("status")}))
    ...
    checks.append(check("manifest_claims_current_candidate", manifest.get("release") == EXPECTED_RELEASE and manifest.get("base") == EXPECTED_BASE and manifest.get("status") == EXPECTED_STATUS, {"release": manifest.get("release"), "base": manifest.get("base"), "status": manifest.get("status")}))
    ...
    checks.append(check("release_doc_manifested", "docs/RELEASE_v0.14.10_CANDIDATE.md" in manifest_paths, {"present": "docs/RELEASE_v0.14.10_CANDIDATE.md" in manifest_paths}))
