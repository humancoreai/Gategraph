...
def main() -> int:
    ...
    checks = []
    checks.append(check("metadata_status_matches_release_token", release == "v0.15.9_STABLE" and base == "v0.15.8_STABLE" and status == "stable", {"release": release, "base": base, "status": status}))
    ...
    for surface in registry.get("current_release_surfaces", []):
        ...
        if surface in {"README.md", "VERSION.md", "RELEASE_STATUS.md", "RELEASE_NOTES.md", "CHANGELOG.md", "RELEASE_METADATA.json", "docs/RELEASE_v0.15.9_STABLE.md"} and base not in text:
