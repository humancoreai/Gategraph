...
def main() -> int:
    ...
    checks = []
    checks.append(check("metadata_status_matches_release_token", release == "v0.16.1_CANDIDATE" and base == "v0.16.0_STABLE" and status == "candidate", {"release": release, "base": base, "status": status}))
    ...
    for surface in registry.get("current_release_surfaces", []):
        ...
        if surface in {"README.md", "VERSION.md", "RELEASE_STATUS.md", "RELEASE_NOTES.md", "CHANGELOG.md", "RELEASE_METADATA.json", "docs/RELEASE_v0.16.1_CANDIDATE.md"} and base not in text:
