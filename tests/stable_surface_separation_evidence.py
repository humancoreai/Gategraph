...
def main() -> int:
    ...
    checks = []
    checks.append(check("metadata_status_matches_release_token", release.endswith("_CANDIDATE") and status == "candidate" and base.endswith("_STABLE"), {"release": release, "base": base, "status": status}))
    ...
    for surface in registry.get("current_release_surfaces", []):
        ...
        if surface in {"README.md", "VERSION.md", "RELEASE_STATUS.md", "RELEASE_NOTES.md", "CHANGELOG.md", "RELEASE_METADATA.json", "docs/RELEASE_v0.16.7_CANDIDATE.md"} and base not in text:
    ...
    checks.append(check("stable_release_surfaces_name_current_release", not missing_release, {"missing": missing_release}))
    checks.append(check("stable_release_surfaces_name_base", not missing_base, {"missing": missing_base}))
