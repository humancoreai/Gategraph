...
EXPECTED_RELEASE = "v0.13.4_CANDIDATE"
EXPECTED_BASE = "v0.13.3_STABLE"
...
def main() -> int:
    ...
    checks = []
    checks.append(check("candidate_release_state", metadata.get("release") == EXPECTED_RELEASE and metadata.get("status") == "candidate", {"release": metadata.get("release"), "status": metadata.get("status")}))
    checks.append(check("candidate_base_is_previous_stable", metadata.get("base") == EXPECTED_BASE, {"base": metadata.get("base")}))
    ...
    checks.append(check("stable_promotion_requires_ci_and_manual_gate", any(t.get("from") == "candidate" and t.get("to") == "stable" and t.get("ci_required") is True and t.get("manual_gate_required") is True for t in allowed), {"allowed_transitions": allowed}))
