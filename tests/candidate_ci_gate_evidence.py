...
def main() -> int:
    ...
    checks = []
    checks.append(check("stable_release_state", metadata.get("release") == EXPECTED_RELEASE and metadata.get("status") in {"candidate", "stable"}, {"release": metadata.get("release"), "status": metadata.get("status")}))
