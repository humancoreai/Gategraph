...
def main() -> int:
    ...
    checks = []
    checks.append(check("candidate_release_state", metadata.get("release") == EXPECTED_RELEASE and metadata.get("status") == "stable", {"release": metadata.get("release"), "status": metadata.get("status")}))
