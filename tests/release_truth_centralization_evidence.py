...
def main() -> int:
    ...
    checks = []
    checks.append(check("metadata_current_release", truth.release == metadata["release"] and truth.base == metadata["base"] and truth.status == metadata["status"], {"release": truth.release, "base": truth.base, "status": truth.status}))
