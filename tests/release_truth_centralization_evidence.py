...
def main() -> int:
    ...
    checks = []
    checks.append(check("metadata_current_candidate", truth.release == "v0.15.6_CANDIDATE" and truth.base == "v0.15.5_STABLE" and truth.status == "candidate", {"release": truth.release, "base": truth.base, "status": truth.status}))
    ...
    checks.append(check(
        ...
        truth.future_stable == "v0.15.6_STABLE"
        and ((truth.status == "candidate" and truth.future_stable != truth.release) or (truth.status == "candidate" and truth.future_stable == truth.release)),
