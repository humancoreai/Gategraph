...
REQUIRED=[
 ...
 'docs/RELEASE_v0.16.7_CANDIDATE.md','docs/FRESH_CLONE_SURFACE_VALIDATION.md','docs/FAILURE_ROOT_CAUSE_GROUPING.md','docs/ARTIFACT_DETERMINISM.md',
...
def main():
    ...
    checks=[]
    checks.append(check('metadata_current_release', meta.get('release')==EXPECTED_RELEASE and meta.get('base')==EXPECTED_BASE and meta.get('status')=='candidate', {'release':meta.get('release'),'base':meta.get('base'),'status':meta.get('status')}))
    checks.append(check('manifest_current_release', manifest.get('release')==EXPECTED_RELEASE and manifest.get('base')==EXPECTED_BASE and manifest.get('status')=='candidate', {'release':manifest.get('release'),'base':manifest.get('base'),'status':manifest.get('status')}))
