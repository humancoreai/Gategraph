...
def main() -> int:
    ...
    check("metadata_current_release", metadata.get("release") == "v0.17.8_STABLE" and metadata.get("status") in {"candidate", "stable"}, {"release": metadata.get("release"), "status": metadata.get("status")}, failures)
