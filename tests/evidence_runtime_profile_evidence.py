...
def main() -> int:
    ...
    check("metadata_current_candidate", metadata.get("release") == "v0.16.6_CANDIDATE" and metadata.get("status") == "candidate", {"release": metadata.get("release"), "status": metadata.get("status")}, failures)
    check("registry_current_candidate", registry.get("release") == metadata.get("release") and registry.get("base") == metadata.get("base") and registry.get("status") == metadata.get("status"), {"release": registry.get("release"), "base": registry.get("base"), "status": registry.get("status")}, failures)
