...
def main() -> int:
    ...
    checks.append(check("manifest_release_matches_current", manifest.get("release") == "v0.16.9_CANDIDATE", {"release": manifest.get("release")}))
    checks.append(check("manifest_status_current", manifest.get("status") in {"candidate", "stable"}, {"status": manifest.get("status")}))
