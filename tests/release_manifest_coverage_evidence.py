...
def main() -> int:
    ...
    checks.append(check("manifest_release_matches_current", manifest.get("release") == "v0.14.10_CANDIDATE", {"release": manifest.get("release")}))
    checks.append(check("manifest_status_stable", manifest.get("status") == "candidate", {"status": manifest.get("status")}))
