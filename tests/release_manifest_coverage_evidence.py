...
def main() -> int:
    ...
    checks.append(check("manifest_release_matches_current", manifest.get("release") == "v0.15.7_STABLE", {"release": manifest.get("release")}))
    checks.append(check("manifest_status_current", manifest.get("status") == "stable", {"status": manifest.get("status")}))
