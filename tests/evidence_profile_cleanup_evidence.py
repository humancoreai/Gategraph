...
def main() -> int:
    ...
    checks.append(check("metadata_current_release", metadata.get("release") == matrix.get("release") and metadata.get("base") == matrix.get("base") and metadata.get("status") == matrix.get("status"), {"release": metadata.get("release"), "base": metadata.get("base"), "status": metadata.get("status")}))
    checks.append(check("matrix_current_release", matrix.get("release") == metadata.get("release") and matrix.get("base") == metadata.get("base") and matrix.get("status") == metadata.get("status"), {"release": matrix.get("release"), "base": matrix.get("base"), "status": matrix.get("status")}))
