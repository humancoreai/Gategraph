...
def main() -> int:
    ...
    for surface in SURFACES:
        ...
        if surface in {"README.md", "VERSION.md", "RELEASE_NOTES.md", "RELEASE_STATUS.md", "RELEASE_METADATA.json", "docs/RELEASE_v0.13.5_CANDIDATE.md"} and EXPECTED_BASE not in text:
            missing_base.append(surface)
        lowered = text.lower()
        if "status: stable" in lowered or '"status": "stable"' in lowered:
            stale_stable_status.append(surface)
    ...
    checks = [
        ...
        check("candidate_surfaces_no_stable_status", not stale_stable_status, {"stale": stale_stable_status}),
