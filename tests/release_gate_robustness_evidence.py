...
def main() -> int:
    ...
    for surface in SURFACES:
        ...
        if surface in {"README.md", "VERSION.md", "RELEASE_NOTES.md", "RELEASE_STATUS.md", "RELEASE_METADATA.json", "docs/RELEASE_v0.13.5_STABLE.md"} and EXPECTED_BASE not in text:
            missing_base.append(surface)
        lowered = text.lower()
        if "status: candidate" in lowered or '"status": "candidate"' in lowered:
            stale_candidate_status.append(surface)
    ...
    checks = [
        ...
        check("stable_surfaces_no_candidate_status", not stale_candidate_status, {"stale": stale_candidate_status}),
