...
def main() -> int:
    ...
    for surface in SURFACES:
        ...
        if surface in {"README.md", "VERSION.md", "RELEASE_NOTES.md", "RELEASE_STATUS.md", "RELEASE_METADATA.json", "docs/RELEASE_v0.17.2_STABLE.md"} and EXPECTED_BASE not in text:
            missing_base.append(surface)
        lowered = text.lower()
        if ('"status": "stable"' not in lowered and 'status: candidate' not in lowered and 'status = "stable"' not in lowered and '# status: candidate' not in lowered):
