...
def main() -> int:
    ...
    assert meta['release']==EXPECTED_RELEASE and meta['base']==EXPECTED_BASE and meta['status']=='stable'
    assert meta['recovery_foundation_scope'] is True and meta['replay_recovery_consistency_scope'] is True
    paths={e['path'] for e in manifest['files']}
    required={'src/recovery_foundation.py','tests/recovery_foundation_evidence.py','tests/replay_recovery_consistency_evidence.py','tests/surface_recovery_consistency_evidence.py','docs/RECOVERY_FOUNDATION.md','docs/RELEASE_v0.14.10_STABLE.md'}
    ...
    assert EXPECTED_RELEASE in read('RELEASE_NOTES.md') and 'Semantic' in read('RELEASE_NOTES.md') and 'Registry' in read('RELEASE_NOTES.md')
