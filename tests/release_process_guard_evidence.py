...
def main() -> int:
    ...
    base = metadata["base"]
    proc = subprocess.run([sys.executable, str(ROOT / "tools" / "release_process_guard.py"), release, status, base], cwd=str(ROOT), text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
    ...
    try:
        proc_git = subprocess.run([sys.executable, str(ROOT / "tools" / "release_process_guard.py"), release, status, base], cwd=str(ROOT), text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
