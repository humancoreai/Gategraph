#!/usr/bin/env python3
"""INV: Stable release surfaces derive the same version/base/status from release metadata."""
...
def main() -> int:
    ...
    assert release == 'v0.16.3_CANDIDATE'
    assert version == '0.16.3'
    assert base == 'v0.16.2_STABLE'
    ...
    if meta.get('status') == 'candidate':
        assert meta.get('future_stable', 'v0.16.3_CANDIDATE') not in readme
