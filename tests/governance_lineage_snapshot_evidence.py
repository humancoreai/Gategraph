...
def _release_order_key(label: str):
    import re
    match = re.search(r"v(\d+)\.(\d+)\.(\d+)", label)
    version = tuple(int(part) for part in match.groups()) if match else (999, 999, 999)
    status_rank = 0 if label.endswith("_STABLE") else 1 if label.endswith("_CANDIDATE") else 2
    return (*version, status_rank)
...
def main() -> int:
    ...
    checks = []
    checks.append(check("lineage_release_stable", data.get("release") == "v0.14.10_CANDIDATE", {"release": data.get("release")}))
    checks.append(check("lineage_base_stable", data.get("base") == "v0.14.9_STABLE", {"base": data.get("base")}))
    ...
    expected_tail = ['v0.12.3_STABLE', 'v0.12.4_STABLE', 'v0.12.5_STABLE', 'v0.12.6_STABLE', 'v0.12.7_STABLE', 'v0.14.4_STABLE', 'v0.14.7_STABLE', 'v0.14.10_CANDIDATE']
