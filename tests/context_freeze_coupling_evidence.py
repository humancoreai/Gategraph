...
EXPECTED_RELEASE='v0.14.10_STABLE'; EXPECTED_BASE='v0.14.9_STABLE'
REQUIRED_FILES={'gategraph/context/context_lifecycle.py','docs/CONTEXT_LIFECYCLE_MODEL.md','tests/context_lifecycle_evidence.py','tests/context_freeze_coupling_evidence.py','tests/context_replay_explain_boundary_evidence.py','docs/RELEASE_v0.14.10_STABLE.md'}
...
def main():
    ...
    check('release_metadata_context_lifecycle_scope', metadata['release']==EXPECTED_RELEASE and metadata['base']==EXPECTED_BASE and metadata['status']=='stable' and metadata.get('context_lifecycle_scope') is True, {'release':metadata.get('release'),'base':metadata.get('base'),'status':metadata.get('status'),'context_lifecycle_scope':metadata.get('context_lifecycle_scope')})
    check('manifest_contains_context_lifecycle_files', manifest['release']==EXPECTED_RELEASE and manifest['base']==EXPECTED_BASE and manifest['status']=='stable' and not (REQUIRED_FILES-paths), sorted(REQUIRED_FILES-paths))
