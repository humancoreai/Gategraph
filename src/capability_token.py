...
DEFAULT_SIGNING_KEY_ID = "local-dev-v1"
_ENV_SECRET = "GATEGRAPH_TOKEN_SIGNING_SECRET"
_DEV_SECRET = "gategraph-local-dev-token-signing-secret"
...
def _signing_secret() -> bytes:
    # SEC: default is deterministic for local tests only; real deployments must inject a secret.
    return os.environ.get(_ENV_SECRET, _DEV_SECRET).encode("utf-8")
...
def sign_claims(*, token_id: str, decision_id: str, task_id: str, capabilities: Dict[str, bool], issued_at: datetime, expires_at: datetime, signing_key_id: str = DEFAULT_SIGNING_KEY_ID) -> str:
    ...
    return hmac.new(_signing_secret(), canonical.encode("utf-8"), hashlib.sha256).hexdigest()
