...
def evaluate_before_task(
    ...
    *,
    ...
    SEC: known session with exhausted budget stops before new per-task budget can hide drift.
    """
    ensure_session_budget_schema(conn)
    ...
    session_cost = _sum_session_cost(conn, session_id)
    agent_cost = _sum_agent_cost(conn, session_id, actor_id)
    ...
    _link_task(conn, session_id, task_id, actor_id)
...
def _link_task(conn: sqlite3.Connection, session_id: str, task_id: str, actor_id: str) -> None:
    now = datetime.now(timezone.utc).isoformat()
    with conn:
        conn.execute(
            """
            INSERT OR IGNORE INTO session_task_links (session_id, task_id, actor_id, created_at)
            VALUES (?, ?, ?, ?)
            """,
            (session_id, task_id, actor_id, now),
        )
...
def _sum_session_cost(conn: sqlite3.Connection, session_id: str) -> int:
    value = conn.execute(
        """
        SELECT COALESCE(SUM(rs.cost_units), 0)
        FROM session_task_links st
        JOIN runtime_steps rs ON rs.task_id = st.task_id
        WHERE st.session_id = ?
        """,
        (session_id,),
    ).fetchone()[0]
...
def _sum_agent_cost(conn: sqlite3.Connection, session_id: str, actor_id: str) -> int:
    value = conn.execute(
        """
        SELECT COALESCE(SUM(rs.cost_units), 0)
        FROM session_task_links st
